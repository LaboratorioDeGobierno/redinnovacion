    ...
    mention: function(options) {
      ...
      var settings = $.extend({}, this.opts, options),
          ...
          for (i in settings.queryBy) {
            if (itemProps[settings.queryBy[i]]) {
              var item = itemProps[settings.queryBy[i]].toLowerCase(),
                doneUsernames = this.query
                  .toLowerCase()
                  .match(
                    new RegExp(settings.delimiter + '[\\w-]+(?:\\s)', 'g')
                  ),
              ...
              if (!!usernames) {
                for (j = 0; j < usernames.length; j++) {
                  if (!!doneUsernames) {
                    var is_done = false;
                    for (var k = 0, l = doneUsernames.length; k < l; k++) {
                      if (doneUsernames[k].indexOf(usernames[j]) !== -1) {
                        is_done = true;
                        break;
                      }
                    }
                  ...
                  var username = usernames[j].substring(1).toLowerCase(),
                    re = new RegExp(settings.delimiter + item + '(\\b|$)', 'g'),
            ...
            if (currentUser.length == 1) {
              for (i = 0; i < len; i++) {
                ...
                if (currentRes.username[0] == currentUser) {
                  ...
                  currentRes.username[0].toLowerCase() ==
        ...
        _highlighter = function(item) {
          // Overriden for compatibility with newer typeahead version
          var query = this.query.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, '\\$&');
          var pattern = new RegExp('(' + query + ')', 'ig');
          return item.replace(pattern, function(_, match) {
